...
function Full() {
  ...
  useEffect(() => {
    ...
    if (products.length > 0 && user.cart.length > 0) {
      let totalPrice = 0;
      for (let i = 0; i < user.cart.length; i++) {
        const { id, quantity } = user.cart[i];
        totalPrice += quantity * products[id - 1].price;
      }
            ...
            {isConnected ? (
              <div>
                <button
                  ...
                  onClick={() => mintToken(currAddress, convertPriceToTokens(totalPrice))}
                >
                  Place Order
                </button>
                {showDialog && (
